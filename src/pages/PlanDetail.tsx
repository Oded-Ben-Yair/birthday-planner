...
import type {
    BirthdayPlan,
    UserInput,
    // BudgetPriorities, // Not directly needed here
    SmartInvitation, // Keep if used for result handling
    Venue,
    Catering,
    GuestEngagement,
    ScheduleItem,
 ...
 * Displays plan details, allows editing, and integrates Budget Optimizer and Invitation Creator Modals.
 */
const PlanDetail: React.FC = () => {
  ...
  const handleOpenOptimizer = () => { if (!plan || !userInput) { setLoadError("Cannot open optimizer: Plan or User Input data is missing."); return; } setOptimizeError(null); setIsOptimizerOpen(true); };
  const handleCloseOptimizer = () => setIsOptimizerOpen(false);
  // Renamed handler to match prop expected by BudgetOptimizer component via the modal wrapper
  const handlePlanUpdateFromOptimizer = (optimizedPlanData: BirthdayPlan) => {
  ...
  const handleOpenInvitationModal = () => { if (!plan) { setLoadError("Cannot create invitation: Plan data is missing."); return; } setInviteError(null); setIsInvitationModalOpen(true); };
  const handleCloseInvitationModal = () => setIsInvitationModalOpen(false);
  // Optional: Handler if invitation modal needs to pass data back up
    ...
    <div className="container mx-auto p-4 md:p-8 max-w-4xl font-inter relative">
      ...
      {displayInviteError}
       ...
       <div className="absolute top-4 right-4 flex flex-wrap gap-2 z-10">
            ...
            <button onClick={handleOpenOptimizer} className="px-4 py-2 bg-purple-600 text-white text-sm font-medium rounded-md shadow-sm hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 transition duration-150 ease-in-out" aria-label="Optimize Budget" disabled={!userInput || !plan}> Optimize Budget </button>
             {/* Create Invitation Button */}
            <button onClick={handleOpenInvitationModal} className="px-4 py-2 bg-teal-600 text-white text-sm font-medium rounded-md shadow-sm hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 transition duration-150 ease-in-out" aria-label="Create Invitation" disabled={!plan}> Create Invitation </button>
       </div>

       {/* Back Button */}
       <button onClick={() => navigate('/results')} className="absolute top-4 left-4 mb-4 px-4 py-2 bg-gray-200 text-gray-800 text-sm font-medium rounded-md shadow-sm hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition duration-150 ease-in-out z-10" aria-label="Back to results"> &larr; Back to Results </button>


      {/* Plan Name & Profile */}
      <div className="flex justify-between items-start mb-6 pb-2 border-b border-gray-300 pt-20"> {/* Increased padding */}
      ...
      <BudgetOptimizerModal
          ...
          currentPlan={plan}
          userInput={userInput}
          // Renamed prop to match what BudgetOptimizer expects
          onPlanUpdate={handlePlanUpdateFromOptimizer}
      />
      ...
      <InvitationCreatorModal
          ...
          currentPlan={plan}
          // Optional: Pass a handler if the modal needs to notify parent on success/error
